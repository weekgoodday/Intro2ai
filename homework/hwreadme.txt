...
hw9:
爬房屋信息
一开始以为很有意思，但其实就是xpath语言在网页上找位置填。（挺浪费时间的不如直接抄了。
在大概猜明白了xpath咋用之后，
...
房价数据处理的一些填空。
真友好，大四老狗太喜欢这样风格的作业了。
出这次作业的助教拉出来表扬，函数写的很优美，我几乎不可能写出来这个框架。
会卡在seaborn boxplot regplot还有pandas的一些函数，group agg qcut有点记不清了。
设计特征分类卡了很久，最后摆烂大多数和参考一样。
另外这次作业实在太长了，中断一下得从头一个个运行。
很惭愧这13个机器学习的方法都不了解，只有几个听说过。
最后的Stack过程没看懂，好像又提了什么stack特征。

hw12:
房价预测 刷榜
开始直接运行 用的RandomForest就有0.3680
更多细节详见实验报告的吐槽，挫败感很强
只能说是一个pandas库练习。